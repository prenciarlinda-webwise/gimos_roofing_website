...
const faqs = [
  ...
  { question: "How often should a Ponte Vedra Beach roof be inspected?", answer: "Annually for tile and slate roofs, plus after every named storm — that's the standard we recommend for all of 32082 and 32081. Tile roofs in The Plantation and the older Sawgrass communities especially need yearly attention because the underlayment beneath the tile fails long before the tile itself, and the only way to catch it early is from inside the attic. We check for cracked or slipped tiles, ridge cap integrity, valley debris, flashing condition around chimneys and skylights, and any signs of water staining on the decking from below. For the larger 5,000-7,000 sq ft Marsh Landing homes, we typically find 8-15 individual issues per inspection that are cheap to fix now and expensive if ignored. Inspections are free for past customers and we keep year-over-year photos on file for comparison." },
